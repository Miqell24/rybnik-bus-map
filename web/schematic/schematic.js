...
const KMK = '#0059a9';
// One colour per operator, plus one for the corridors they share. Picked to stay
// apart on white paper and to survive being printed: navy, green, orange, plum,
// and a strong red for "more than one operator here".
const BADGE_IMAGES = []; // [name, ImageData, opts] — replayed onto the export map
const OP_COLOR = [
  'match', ['get', 'op'],
  'KM Rybnik', '#0059a9',
  'BKM Żory', '#0f8a4a',
  'Wodzisław county', '#c86a00',
  'MZK Jastrzębie + region', '#7d2b8b',
  'mix', '#c8102e',
  '#5a6672',
];
const TRAM = '#d6212b';
...
async function init() {
  ...
  document.getElementById('counts').textContent =
    `${c.busLines} bus + ${c.tramLines} tram lines · ${c.stations} stations · ` +
  ...
  map.addControl(new maplibregl.AttributionControl({
    compact: true, customAttribution: 'Diagram: GTFS KM Rybnik · BKM Żory · MZK Jastrzębie · powiat wodzisławski',
  }));
  ...
  map.on('load', () => {
    ...
    map.addLayer({
      ...
      paint: {
        ...
        'circle-color': ['case', ['==', ['get', 'tram'], 1], TRAM, KMK],
    ...
    // the filled blocks were the loudest thing on the sheet (user verdict on
    // the Warsaw diagram, 22.08.2026); the printed map's badges are white.
    for (const [name, fill, rim] of [['badge-bus', 'rgba(255,255,255,0.92)', '#0059a9'], ['badge-tram', 'rgba(255,255,255,0.92)', '#d6212b']]) {
    ...
    const terminalDef = (id, extra) => ({
      ...
      layout: {
        'icon-image': ['concat', 'badge-', ['get', 'mode']],
        'icon-text-fit': 'both',
      ...
      paint: { 'text-color': ['match', ['get', 'mode'], 'tram', TRAM, KMK] },
      ...extra,
    });
    // plates arrive later and the early wave is rarer: at overview zoom the
    // network should read through its corridors, not its loops
      ...
      map.on('click', layerId, (e) => {
        ...
        new maplibregl.Popup({ closeButton: false, offset: 10 })
          ...
          .setHTML(`<strong>${p.name}</strong><br>${p.n} line${p.n === 1 ? '' : 's'} · ` +
            `${['bus', 'tram'].filter((m) => p[m]).join(' + ')}${p.node ? ' · layout node' : ''}`)
          .addTo(map);
      });
...
function drawLegend(ctx, x, y, meta, scale) {
  ...
  for (const [name, color] of OPS) {
    line(color, 'Linia — ' + name, name === 'shared by 2+ operators'
      ? 'Corridor used by more than one operator' : 'Line of ' + name);
  }
  ...
  ctx.fillText(`GTFS: KM Rybnik · BKM Żory · MZK Jastrzębie-Zdrój · powiat wodzisławski · ${when}`,
    x + pad, y + bh - 16 * s);
  ctx.restore();
}
...
async function exportPng() {
  ...
  a.download = `rybnik-diagram_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${sheet.width}x${sheet.height}.png`;
