...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: KM Rybnik · BKM Żory · WKP · zbiorkom.live GTFS' }));
  ...
  document.getElementById('count').textContent = `(${nBus} bus lines · 5 operators)`;
  ...
  const paintChips = (linesView) => {
    const OPS = [['ryb', 'KM Rybnik'], ['zor', 'BKM Żory'], ['wod', 'Wodzisław county — WKP'],
      ['jas', 'Jastrzębie-Zdrój & the regional bundle']];
    ...
    const section = (key, title) => {
      ...
      return `<h3 class="chip-head">${esc(title)} <span class="n">${ls.length}</span></h3>` +
        `<div class="chip-cloud">${ls.map((l) => chipHtml(l, linesView ? lineColor(l.line) : l.color, l.line === state.selected)).join(' ')}</div>`;
    };
    let html = OPS.map(([k, t]) => section(k, t)).join('');
    for (const k of [...bucket.keys()]) html += section(k, k);
  ...
  const PALETTE = [
    ...
    ['#0088ce', '#00466b'], // Cairo Metro M1 — missing here meant M1 badges
                            // rendered as bare numbers and M1 station dots vanished
  ];
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: KM Rybnik, BKM Żory, WKP, zbiorkom.live';
      ...
      a.download = `rybnik-region-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `rybnik-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): single ZTM lines reach Pyskowice, Sławków and
  // Bieruń, so fitting the data bbox would open on countryside with the
  // region as a stripe. Open on the central Cairo–Giza core instead.
  map.jumpTo({ center: [18.52, 50.04], zoom: 10.6 });
}
