...
// One mode: buses of the Rybnik region (Subregion Zachodni) — FOUR feeds merged
// into a single network: KM Rybnik, BKM Żory, Wodzisławska Komunikacja
// Powiatowa (W-prefixed line keys — the plain numbers collide with KM Rybnik)
// and the zbiorkom.live regional bundle (MZK Jastrzębie-Zdrój + the Wodzisław
// 20x city shuttles + the Czerwionka-Leszczyny 30x network; its copies of the
// Żory lines are dropped in favour of the official Żory feed).
// Usage: node pipeline/build.mjs [--all | lines...]
// Quirks: only the Jastrzębie bundle has shapes.txt — the other feeds' routes
// are matched from their stop sequences (per-rep pseudo mode).
...
// m — longer jumps between shape points are GTFS data gaps (applies to the
// Jastrzębie feed, the only one with real shapes; the others use stop-sequence
// pseudo matching where every leg is long by nature and this threshold is
// deliberately not used). Inside a real gap the HMM bridges by routing instead
// of interpolating observations, which would fabricate straight-line detours.
const GAP_MIN = 300;
...
// FOUR feeds, ONE bus network (Rybnik region — Subregion Zachodni): every feed
// loads under its own tag (IDs namespaced, so nothing collides downstream) and
...
const MODES = [{
  ...
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['1']),
  feeds: [
    // "-->" = depot/positioning runs ("Wyjazd na linię", "JADA NA SZYCHTA") —
    // not a passenger line
    { tag: 'ryb', dir: 'data/gtfs-rybnik', mapKey: (sn) => (sn === '-->' ? null : sn) },
    { tag: 'zor', dir: 'data/gtfs-zory', mapKey: (sn) => sn },
    // powiat lines are plain numbers colliding with KM Rybnik (32, 33, 34, 36,
    ...
    { tag: 'jas', dir: 'data/gtfs-jastrzebie', mapKey: (sn) => (/^0[1-9]$|^ZL$/.test(sn) ? null : sn) },
  ],
}];

// Feed coordinate fixes: poles the GTFS places on the wrong street. Keyed by
// `<feed tag>:<stop_id>`, values are the coordinates of that stop's node in
// OSM. These feeds have no shapes, so the stop sequence IS the matching
// observation — a misplaced pole does not just draw the dot in the wrong
// place, it drags the whole route into a detour there.
//   ryb:557/558 "Jankowice Kościelna" (codes 269/0 and 269/1) — the feed puts
//   both poles ~250 m west of the Rybnicka junction, on Podkościele; line 30
//   was drawn making a U-turn into that side street (user report). OSM has the
//   poles where they stand: on Kościelna, east of the junction.
//   ryb:559/560 "Jankowice Kościół" (270/0, 270/1) — same street, same fault:
//   ~60 m off Rybnicka, enough to hook the line into the service road there.
const STOP_FIX = {
  'ryb:557': [50.047889, 18.550815],
  'ryb:558': [50.047783, 18.550846],
  'ryb:559': [50.046321, 18.548033],
  'ryb:560': [50.046285, 18.547834],
};

// Runs of the same line set are welded into one chain — but only within one
// STREET NAME. Merging across names used to produce chains that ran through a
// junction into the next street and carried the first segment's name for the
// whole length, so the map printed "Kościelna" along Rybnicka and hung the line
// numbers of three streets on one anchor (user report).
...
async function processMode(cfg) {
  ...
  cfg.trolleySet = new Set(); // no trolleybuses or metrolines in this region
  ...
  for (const feed of cfg.feeds) {
    ...
    // Rybnik and Żory ship shapes.txt as a lone header row — treat as absent
    ...
    // direction_id alone can lie (the Athens/OSY defect): the Wodzisław feed
    // ships the column EMPTY on all 583 trips (9 of its 12 lines lost the
    // return leg), and single lines elsewhere (L-2 Jastrzębie, 12/N8 Rybnik,
    // 01 Żory) carry one constant value on all trips while the return exists
    // as separate trips with a mirrored headsign. Grouped by direction_id
    // alone both directions collapse into one bucket and only the busier one
    // gets drawn. Pre-scan direction diversity per line; where direction_id
    // cannot tell directions apart, the headsign is the key.
  ...
  for (const r of reps) {
    ...
    let sampled, opts;
    if (r.pseudo) {
      // BUS stops as sparse observations (Rybnik/Żory/Wodzisław ship no shape
      // geometry): poles sit roadside, sometimes in bays — wider net and softer
      // emission than shape matching, soft beta because consecutive stops are
      // 300–600 m apart and the routing between them IS the geometry. gapMin
      // stays OFF (Infinity): with routing doing the drawing, the oneway
      // penalties are exactly what keeps dual carriageways directional.
      sampled = xy;
...
await (await import('./night.mjs')).nightPass(outDir, /^N\d/, { sort: true });
